...
<body> 
      ...
      <div class="form-group"> 
        ...
        <div class="col-sm-10"> 
          ...
          <!-- Input box to enter the  
              required data -->
          <select id="content" class="form-control"  name="CodC">
            <?php
            ...
            $sql = 'SELECT CourseID, CourseName, ClassDate, ClassTime FROM course WHERE PersonID = \''.$row1[0].'\'';
            ...
            while ($row = mysqli_fetch_row($result)) {
              echo '<option id = "content" value=\''.$row[0].'\'>'.$row[1].' - '.$row[2].' - '.$row[3].'</option>';
            }
          ?>
        </select> 
        </div> 
  ...
  <script> 
    ...
    function htmlEncode(value) { 
      return "localhost/university/QR-attendance/QRdisplay.php/?curs=" + $('<div/>').text(value) 
        .html(); 
    } 
